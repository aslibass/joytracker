...
async def home(request: Request, db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse(request, "feed.html", {"user": user, "entries": entries})
...
async def get_entry_status(request: Request, entry_id: int, db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse(request, "components/joy_entry_card.html", {"entry": entry})
...
async def toggle_admin(request: Request, user_id: int, db: Session = Depends(get_db), admin: models.User = Depends(get_current_admin_user)):
    ...
    return templates.TemplateResponse(request, "components/user_row.html", {
        "u": target_user
    })
